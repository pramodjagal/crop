...
var displaySizeName = "4x6"; //Todo: change it - hard coded for now
...
if (hiresWidth <= 640 || hiresHeight <= 640) {
    $(landscapeRadio).style.display = "none";
    $(landscapeImageRadio).style.display = "none";
    $(potraitRadio).style.display = "none";
    $(potraitImageRadio).style.display = "none";
}
...
function doneEditing() //called when the 'done editing' button is clicked
{
    ...
    var isSingleSizeSelected = true;
    if (isSingleSizeSelected) {
        var pictOwnerId = "10000282001_10003358001"; //todo: change this to pictId_ownerId - hardcoded now
        var cropPictSize = "reprint;4x6"; // todo: change this to upc - harcoded now
